...
class Spaam(object):
    ...
    def _pei_t_t1(self, dmu, idx, is_t=True):
        if is_t:
            return (dmu.ene.total / dmu.pro.production) / \
                    sqrt(self._lmdi.phi_t_t[idx] * self._lmdi.phi_t1_t[idx])
        else:
            return (dmu.ene.total / dmu.pro.production) / \
                    sqrt(self._lmdi.phi_t1_t1[idx] * self._lmdi.phi_t_t1[idx])
    def rpei(self):
        '''
        r's pei
        '''
    ...
    def _pis_t_t1(self, dmu, idx, is_t=True):
        if is_t:
            return dmu.pro.production * sqrt(self._lmdi.eta_t_t[idx] *
                                             self._lmdi.eta_t1_t[idx]) / self._lmdi.y_t
        else:
            return dmu.pro.production * sqrt(self._lmdi.eta_t1_t1[idx] *
                                             self._lmdi.eta_t_t1[idx]) / self._lmdi.y_t1
    def rpis(self):
        '''
        r's pis
        '''
    ...
    def _eue_t_t1(self, dmu, idx, is_t=True):
        if is_t:
            return self._lmdi.phi_t_t[idx]
        else:
            return self._lmdi.phi_t1_t1[idx]
    def reue(self):
        '''
        r's eue
        '''
    ...
    def _est_t_t1(self, dmu, idx, is_t=True):
        if is_t:
            return sqrt(self._lmdi.phi_t1_t[idx] / self._lmdi.phi_t_t[idx])
        else:
            return sqrt(self._lmdi.phi_t_t1[idx] / self._lmdi.phi_t1_t1[idx])
    def rest(self):
        '''
        r's est
        '''
    ...
    def _yct_t_t1(self, dmu, idx, is_t=True):
        if is_t:
            return sqrt(self._lmdi.eta_t_t[idx] / self._lmdi.eta_t1_t[idx])
        else:
            return sqrt(self._lmdi.eta_t1_t1[idx] / self._lmdi.eta_t_t1[idx])
    def ryct(self):
        '''
        r yct
        '''
...
if __name__ == '__main__':
    ...
    spaam_2006_2007 = Spaam(dmus_2006, dmus_2007, '2006-2007')
    spaam_2006_2007.rcef()
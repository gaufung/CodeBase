...
class Lmdi(object):
    ...
    def _init_linear_programming(self):
        ...
        '''
        self._phi_t_t = phi_min([self._dmus_t, ], self._dmus_t, True)
        self._phi_t_t1 = phi_min([self._dmus_t, ], self._dmus_t1, False)
        self._phi_t1_t = phi_min([self._dmus_t1, ], self._dmus_t, False)
        self._phi_t1_t1 = phi_min([self._dmus_t1, ], self._dmus_t1, True)
        self._eta_t_t = eta_max([self._dmus_t, ], self._dmus_t, True)
        self._eta_t_t1 = eta_max([self._dmus_t, ], self._dmus_t1, False)
        self._eta_t1_t = eta_max([self._dmus_t1, ], self._dmus_t, False)
        self._eta_t1_t1 = eta_max([self._dmus_t1, ], self._dmus_t1, True)
    ...
    def _init_y_sum(self):
        ...
        self._y_t1 = reduce(operator.add, [self._dmus_t1[k].pro.production * sqrt(
            self._eta_t1_t1[k] * self._eta_t_t1[k]) for k in range(self._province_count)])
        self._y_t = reduce(operator.add, [self._dmus_t[k].pro.production * sqrt(
            self._eta_t_t[k] * self._eta_t1_t[k]) for k in range(self._province_count)])
    ...
    @property
    def phi_t_t(self):
        return self._phi_t_t

    @property
    def phi_t_t1(self):
        return self._phi_t_t1

    @property
    def phi_t1_t(self):
        return self._phi_t1_t

    @property
    def phi_t1_t1(self):
        return self._phi_t1_t1
    ...
    @property
    def eta_t_t1(self):
        return self._eta_t_t1

    @property
    def eta_t1_t(self):
        return self._eta_t1_t
    ...
    def _pei(self, dmu_t, dmu_t1, j, i):
        ...
        if dmu_t.ene[j] != 0.0 and dmu_t1.ene[j] != 0.0:
            number1 = dmu_t1.ene.total / \
                sqrt(self.phi_t1_t1[
                    i] * self.phi_t_t1[i]) / dmu_t1.pro.production
            number2 = dmu_t.ene.total / \
                sqrt(self.phi_t1_t[i] *
                     self.phi_t_t[i]) / dmu_t.pro.production
    ...
    def _pis(self, dmu_t, dmu_t1, j, i):
        ...
        if dmu_t.ene[j] != 0.0 and dmu_t1.ene[j] != 0.0:
            number1 = dmu_t1.pro.production * \
                sqrt(self.eta_t1_t1[i] * self.eta_t_t1[i]) / self.y_t1
            number2 = dmu_t.pro.production * \
                sqrt(self.eta_t1_t[i] * self.eta_t_t[i]) / self.y_t
            numerator1 = log(number1 / number2)
            number3 = dmu_t1.co2[j] / self.co2_sum_t1
            number4 = dmu_t.co2[j] / self.co2_sum_t
    ...
    def _eue(self, dmu_t, dmu_t1, j, i):
        ...
        '''
        if dmu_t.co2[j] != 0.0 and dmu_t1.co2[j] != 0.0:
            numerator1 = log(self.phi_t1_t1[i] / self.phi_t_t[i])
            number3 = dmu_t1.co2[j] / self.co2_sum_t1
            number4 = dmu_t.co2[j] / self.co2_sum_t
            numerator2 = Lmdi.l_function(number3, number4)
            return numerator1 * numerator2 / (self.ll)
    ...
    def _est(self, dmu_t, dmu_t1, j, i):
        ...
        '''
        if dmu_t.co2[j] != 0.0 and dmu_t1.co2[j] != 0.0:
            number1 = sqrt(self.phi_t_t1[i] / self.phi_t1_t1[i])
            number2 = sqrt(self.phi_t1_t[i] / self.phi_t_t[i])
            numerator1 = log(number1 / number2)
            number3 = dmu_t1.co2[j] / self.co2_sum_t1
            number4 = dmu_t.co2[j] / self.co2_sum_t
            numerator2 = Lmdi.l_function(number3, number4)
    ...
    def _yct(self, dmu_t, dmu_t1, j, i):
        ...
        '''
        if dmu_t.co2[j] != 0.0 and dmu_t1.co2[j] != 0.0:
            number1 = sqrt(self.eta_t1_t1[i] / self.eta_t_t1[i])
            number2 = sqrt(self.eta_t_t[i] / self.eta_t1_t[i])
            numerator1 = log(number1 / number2)
            number3 = dmu_t1.co2[j] / self.co2_sum_t1
            number4 = dmu_t.co2[j] / self.co2_sum_t
            numerator2 = Lmdi.l_function(number3, number4)
...
#  各个省份的能源指数分解合并
    ...
    def _pei_province(self, dmu_t, dmu_t1, idx, j):
        number1 = dmu_t1.ene.total / \
                sqrt(self.phi_t1_t1
                     [idx] * self.phi_t_t1[idx]) / dmu_t1.pro.production
        number2 = dmu_t.ene.total / \
                sqrt(self.phi_t1_t[idx] *
                     self.phi_t_t[idx]) / dmu_t.pro.production
        numerator1 = log(number1 / number2)
    ...
    def _eue_province(self, dmu_t, dmu_t1, idx, j):
        numerator1 = log(self.phi_t1_t1[idx] / self.phi_t_t[idx])
        numerator2 = self._l_i_j(idx, j)
        return numerator1 * numerator2 / self._ll_i(idx)
    ...
    def _est_province(self, dmu_t, dmu_t1, idx, j):
        number1 = sqrt(self.phi_t_t1[idx] / self.phi_t1_t1[idx])
        number2 = sqrt(self.phi_t1_t[idx] / self.phi_t_t[idx])
        numerator1 = log(number1 / number2)
        numerator2 = self._l_i_j(idx, j)
        return numerator1 * numerator2 / self._ll_i(idx)

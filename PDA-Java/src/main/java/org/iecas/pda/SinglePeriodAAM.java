...
public class SinglePeriodAAM implements Serializable{
    ...
    //rei
    private double reiTT1(Dmu dmu, int i, boolean isT){
        return dmu.getEnergy().total()/dmu.getProduction().getProduction();
    }
    public List<Double> rrei()
    {
        List<Double> result = new ArrayList<>();
        double _rei = exp(lmdi.rei().stream().mapToDouble(i->i).sum());
        for(int i =0;i<lmdi.getProvinceCount();i++){
            double _wi = wi(i);
            double reiT = reiTT1(dmusT.get(i),i,true);
            double reiT1 = reiTT1(dmusT1.get(i),i,false);
            double pii = _wi/lmdi.lFunction(reiT1, reiT*_rei);
            result.add(pii*reiT);
        }
        double total = result.stream().mapToDouble(i->i).sum();
        return result.stream().mapToDouble(i->i/total).boxed().collect(Collectors.toList());
    }
    public List<Double> reiRatio(){
        List<Double> result = new ArrayList<>();
        for(int i =0;i<lmdi.getProvinceCount();i++){
            double reiT = reiTT1(dmusT.get(i),i,true);
            double reiT1  = reiTT1(dmusT1.get(i),i,false);
            result.add(reiT1/reiT-1);
        }
        return result;
    }
    public List<Double> reiAttributions() {
        List<Double> rrei =rrei();
        List<Double> reiRatio = reiRatio();
        return IntStream.range(0,lmdi.getProvinceCount()).
                mapToDouble(i->rrei.get(i)*reiRatio.get(i)).
                boxed().collect(Collectors.toList());
    }
    //ros

    private double rosTT1(Dmu dmu, int i, boolean isT){
        if(isT)
            return dmu.getProduction().getProduction()/lmdi.getProductionSumT();
        else
            return dmu.getProduction().getProduction()/lmdi.getProductionSumT1();
    }

    public List<Double> rros(){
        List<Double> result = new ArrayList<>();
        double _ros = exp(lmdi.ros().stream().mapToDouble(i->i).sum());
        for(int i=0; i<lmdi.getProvinceCount();i++){
            double __wi = wi(i);
            double rosT = rosTT1(dmusT.get(i),i,true);
            double rosT1 = rosTT1(dmusT1.get(i),i,false);
            double pii = __wi/lmdi.lFunction(rosT1, rosT*_ros);
            result.add(pii*rosT);
        }
        double total = result.stream().mapToDouble(i->i).sum();
        return result.stream().mapToDouble(i->i/total).boxed().collect(Collectors.toList());
    }
    public List<Double> rosRatio()
    {
        List<Double> result =new ArrayList<>();
        for(int i=0;i<lmdi.getProvinceCount();i++){
            double yoeT = rosTT1(dmusT.get(i),i,true);
            double yoeT1 = rosTT1(dmusT1.get(i),i, false);
            result.add(yoeT1/yoeT-1);
        }
        return result;
    }
    public List<Double> rosAttributions()
    {
        List<Double> rros = rros();
        List<Double> rosRatio = rosRatio();
        return IntStream.range(0,lmdi.getProvinceCount()).
                mapToDouble(i->rros.get(i)*rosRatio.get(i)).
                boxed().collect(Collectors.toList());
    }
    //gdp
    private double gdpTT1(Dmu dmu, int i, boolean isT){
        if(isT)
            return lmdi.getProductionSumT();
        else
            return lmdi.getProductionSumT1();
    }
    public List<Double> rgdp(){
        List<Double> result = new ArrayList<>();
        double _ros = exp(lmdi.gdp().stream().mapToDouble(i->i).sum());
        for(int i=0; i<lmdi.getProvinceCount();i++){
            double __wi = wi(i);
            double rosT = gdpTT1(dmusT.get(i),i,true);
            double rosT1 = gdpTT1(dmusT1.get(i),i,false);
            //double pii = __wi/lmdi.lFunction(rosT1, rosT*_ros);
            double pii = __wi/rosT1;
            result.add(pii*rosT);
    ...
    public List<Double> gdpRatio()
    {
        List<Double> result =new ArrayList<>();
        for(int i=0;i<lmdi.getProvinceCount();i++){
            double gdpT = gdpTT1(dmusT.get(i),i,true);
            double gdpT1 = gdpTT1(dmusT1.get(i),i, false);
            result.add(gdpT1/gdpT-1);
        }
        return result;
    }
    public List<Double> gdpAttributions()
    {
        List<Double> rgdp = rgdp();
        //System.out.println(Arrays.toString(rgdp.toArray()));
        List<Double> gdpRatio = gdpRatio();
        System.out.println(Arrays.toString(gdpRatio.toArray()));
        return IntStream.range(0,lmdi.getProvinceCount()).
                mapToDouble(i->rgdp.get(i)*gdpRatio.get(i)).
                boxed().collect(Collectors.toList());
    }
    ...
    public double getyct() { return exp(lmdi.yct().stream().mapToDouble(i->i).sum());}
    public double getrei() { return exp(lmdi.rei().stream().mapToDouble(i->i).sum());}
    public double getros() { return exp(lmdi.ros().stream().mapToDouble(i->i).sum());}
    public double getgdp() { return exp(lmdi.gdp().stream().mapToDouble(i->i).sum());}

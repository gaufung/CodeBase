...
//    private Double[][] cefT1;
    ...
    private double rei(Dmu dmuT, Dmu dmuT1, int j, int i){
        if(dmuT.getEnergy().energyAt(j)!=0.0 && dmuT1.getEnergy().energyAt(j)!=0.0) {
            double number1 = dmuT1.getEnergy().total()/dmuT1.getProduction().getProduction();
            double number2 = dmuT.getEnergy().total()/dmuT.getProduction().getProduction();
            double numerator1 = log(number1/number2);
            double number3 = dmuT1.getCo2().co2At(j)/this.co2SumT1;
            double number4 = dmuT.getCo2().co2At(j)/this.co2SumT;
            double numerator2 = lFunction(number3, number4);
    ...
    public List<Double> rei(){
        List<Double> rei = new ArrayList<>(this.provinceCount);
        for(int i =0; i<this.provinceCount;i++){
            double result =0;
            for(int j = 0; j<this.energyCount;j++){
                double temp = rei(this.dmusT.get(i), this.dmusT1.get(i),j,i);
                result += temp;
            }
            rei.add(result);
        }
    ...
    private double ros(Dmu dmuT, Dmu dmuT1, int j, int i){
        if(dmuT.getEnergy().energyAt(j)!=0.0 && dmuT1.getEnergy().energyAt(j)!=0.0) {
            double number1 = dmuT1.getProduction().getProduction()/productionSumT1;
            double number2 = dmuT.getProduction().getProduction()/productionSumT;
            double numerator1 = log(number1/number2);
            double number3 = dmuT1.getCo2().co2At(j)/this.co2SumT1;
            double number4 = dmuT.getCo2().co2At(j)/this.co2SumT;
            double numerator2 = lFunction(number3, number4);
    ...
    public List<Double> ros() {
        List<Double> ros = new ArrayList<>(this.provinceCount);
        for(int i=0; i<this.provinceCount;i++){
            double result =0.0;
            for(int j=0; j<this.energyCount;j++){
                result += ros(this.dmusT.get(i), this.dmusT1.get(i),j,i);
            }
            ros.add(result);
        }
        return ros;
    ...
    private double gdp(Dmu dmuT, Dmu dmuT1, int j, int i){
        if(dmuT.getEnergy().energyAt(j)!=0.0 && dmuT1.getEnergy().energyAt(j)!=0.0) {
            double number1 = productionSumT1;
            double number2 = productionSumT;
            double numerator1 = log(number1/number2);
            double number3 = dmuT1.getCo2().co2At(j)/this.co2SumT1;
            double number4 = dmuT.getCo2().co2At(j)/this.co2SumT;
            double numerator2 = lFunction(number3, number4);
    ...
    public List<Double> gdp(){
        List<Double> gdp = new ArrayList<>(this.provinceCount);
        for(int i =0; i<this.provinceCount;i++){
            double result = 0.0;
            for(int j =0;j<this.energyCount;j++){
                result += gdp(this.dmusT.get(i), this.dmusT1.get(i),j,i);
            }
            gdp.add(result);
        }
        return gdp;

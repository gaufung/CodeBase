...
public class TestMultiPeriodAAM  extends TestCase{
    ...
    public void testRei() throws Exception{
        double actual = mpaam.getIndexT(8,"rei");
        double expect = mpaam.rei().stream().mapToDouble(i->i).sum();
        assertEquals(actual-1, expect, 0.0001);
    }
    public void testRos() throws Exception{
        double actual = mpaam.getIndexT(8,"ros");
        double expect = mpaam.ros().stream().mapToDouble(i->i).sum();
        assertEquals(actual-1, expect, 0.0001);
    }
    public void testGdp() throws Exception{
        double actual = mpaam.getIndexT(8,"gdp");
        double expect = mpaam.gdp().stream().mapToDouble(i->i).sum();
        assertEquals(actual-1, expect, 0.0001);
    }
}
